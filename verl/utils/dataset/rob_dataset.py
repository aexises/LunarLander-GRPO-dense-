...
class LunarLanderDataset(Dataset):
    def __init__(self, num_trials_per_task=256, train_val="train", eval_seed_list=None):
        self.num_trials_per_task = num_trials_per_task
        self.train_val = train_val
        self.eval_seed_list = list(eval_seed_list or [])
        self._read_files_and_tokenize()

    def _read_files_and_tokenize(self):
        dataframes = []
        if self.train_val == "train":
            seeds = list(range(int(self.num_trials_per_task)))
            data_source = "lunarlander_train"
        else:
            seeds = self.eval_seed_list or list(range(int(self.num_trials_per_task)))
            data_source = "lunarlander_eval"

        for seed in seeds:
            dataframes.append({
                "task_suite_name": "lunarlander",
                "task_id": torch.tensor(0, dtype=torch.int64).unsqueeze(0),
                "trial_id": torch.tensor(int(seed), dtype=torch.int64).unsqueeze(0),
                "trial_seed": torch.tensor(int(seed), dtype=torch.int64).unsqueeze(0),

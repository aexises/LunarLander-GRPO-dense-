...
def compute_data_metrics(batch,config):
    ...
    if 'r_total' in batch.batch.keys():
        success = batch.batch.get('success', batch.batch['complete']).float()
        crash = batch.batch.get('crash', torch.zeros_like(success, dtype=torch.bool)).float()
        metrics.update({
            ...
            'critic/task/mean_abs_final_theta': batch.batch.get('final_theta', torch.zeros_like(success)).abs().float().mean().detach().item(),
        })
    return metrics
...
class RayTrainer(object):
    ...
    def _create_dataloader(self):   # next fix
        ...
        from verl.utils.dataset.rob_dataset import LIBERO_Dataset, LunarLanderDataset, Robotwin_Dataset, collate_fn
        ...
        elif self.config.data.task_suite_name == "lunarlander":
            self.train_dataset = LunarLanderDataset(
                num_trials_per_task=self.config.data.num_trials_per_task,
                train_val="train",
                eval_seed_list=self.config.eval.seed_list,
            )
            self.val_dataset = LunarLanderDataset(
                num_trials_per_task=max(len(self.config.eval.seed_list), 1),
                train_val="valid",
                eval_seed_list=self.config.eval.seed_list,
            )
        else:
    ...
    def _validate(self, global_steps=0):
        ...
        for test_data in self.val_dataloader:
            ...
            for k, v in reward_metrics.items():
                metric_accumulator['test_reward/' + k].append(v)
                
            for k, v in format_metrics.items():
                metric_accumulator['format_acc/' + k].append(v)
                
            for k, v in reward_format_metrics.items():
                metric_accumulator['acc_wformat/' + k].append(v)
        ...
        metric_dict = {key: float(np.mean(values)) for key, values in metric_accumulator.items()}
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            while True:
                ...
                with Timer(name='logging2', text="{name}: {seconds:.1f} seconds") as timer:
                    ...
                    if 'train_reward/mean_total_reward' in metrics and 'train_reward/success_rate' in metrics:
                        warning_flag = 0.0
                        if self._last_alignment_snapshot is not None:
                            if (
                                metrics['train_reward/mean_total_reward'] > self._last_alignment_snapshot['mean_total_reward']
                                and metrics['train_reward/success_rate'] <= self._last_alignment_snapshot['success_rate']
                            ):
                                warning_flag = 1.0
                                print('Warning: shaped reward increased while success rate stayed flat or decreased.')
                        metrics['diagnostics/reward_hacking_warning'] = warning_flag
                        self._last_alignment_snapshot = {
                            'mean_total_reward': metrics['train_reward/mean_total_reward'],
                            'success_rate': metrics['train_reward/success_rate'],
                        }
